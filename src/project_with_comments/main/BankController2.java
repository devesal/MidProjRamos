...
import project_with_comments.models.BankAccount2;
import project_with_comments.models.CheckingAccount2;
import project_with_comments.models.CreditCardAccount2;
import project_with_comments.models.InvestmentAccount2;
...
public class BankController2 {
    ...
    private final ArrayList<BankAccount2> bankAccounts = new ArrayList<>();
    ...
    public void start() {
        ...
        while (true) {
            ...
            while (staySignedIn) {
                if (currentAccount instanceof CheckingAccount2) {
                    staySignedIn = displayCheckingAccMenu();
                } else if (currentAccount instanceof CreditCardAccount2) {
                    staySignedIn = displayCCAccMenu();
                } else if (currentAccount instanceof InvestmentAccount2) {
    ...
    private boolean findAccount(int accountNo) {
        for (BankAccount2 account : bankAccounts) {
    ...
    private boolean displayCCAccMenu() {
        ...
        System.out.println("\n" + currentAccount.getAccountName());
        System.out.println(currentAccount.getAccountNo() + " - " + currentAccount.displayAccountType());
        System.out.println("Credit Limit: ₱" + ((CreditCardAccount2) currentAccount).getCreditLimit() + "\n");
    ...
    private boolean selectCheckingAccOptions() {
        switch (selectMenuOption(7)) {
            ...
            case 4 -> ((CheckingAccount2)currentAccount).encashCheck(getDoubleInput("Enter amount to encash: "));
            case 5 -> System.out.println(currentAccount.toString());
            case 6 -> {
                currentAccount.closeAccount(bankAccounts);
                goBack();
    ...
    private boolean selectCCAccOptions() {
        switch (selectMenuOption(7)) {
            case 1 -> ((CreditCardAccount2) currentAccount).inquireAvailableCredit();
            case 2 -> ((CreditCardAccount2) currentAccount).getCashAdvance(getDoubleInput("Enter cash advance amount: "));
            case 3 -> ((CreditCardAccount2) currentAccount).payCard(getDoubleInput("Enter amount to pay: "));
            case 4 -> ((CreditCardAccount2) currentAccount).chargeToCard(getDoubleInput("Enter amount to charge: "));
            case 5 -> System.out.println(currentAccount.toString());
            case 6 -> {
                currentAccount.closeAccount(bankAccounts);
                goBack();
    ...
    private boolean selectInvestmentAccOptions() {
        switch (selectMenuOption(6)) {
            case 1 -> balanceInquiry();
            case 2 -> ((InvestmentAccount2) currentAccount).addInvestment(getDoubleInput("Enter amount to invest: "));
    ...
    private boolean displayInvestmentAccMenu() {
        ...
        System.out.println("Minimum Balance: ₱" + ((InvestmentAccount2) currentAccount).getMinimumBalance());
    ...
    private void createAccount(String type) {
        ...
        switch (type) {
            case "Savings" -> bankAccounts.add(new BankAccount2(accountNo, name));
            case "Checking" -> bankAccounts.add(new CheckingAccount2(accountNo, name,  0.0));
            case "Credit" -> bankAccounts.add(new CreditCardAccount2(accountNo, name, 25000, 0));
            case "Investment" -> bankAccounts.add(new InvestmentAccount2(accountNo, name, 500, 0.35));
        }
    ...
    private void inquireInvestmentValue() {
        ...
        double investmentValue = ((InvestmentAccount2) currentAccount).inquireInvestmentValue();
    ...
    public static void main(String[] args) {
        BankController2 bankController2 = new BankController2();
        bankController2.start();
    }
}
